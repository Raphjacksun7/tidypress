...
Presets exist so the tree already looks like a public site — writing on the feed, work on the home page — instead of a generic `content/` dump. After `tidypress build`, `build/llms.txt` gives the model the full published corpus **without** giving it custody of a separate snapshot file in the repo.
