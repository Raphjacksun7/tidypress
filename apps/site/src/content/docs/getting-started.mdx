...
- **`build/`** — HTML, assets, Pagefind search, `llms.txt` (full published markdown for agents), and sitemap when `siteUrl` is production-ready. Upload only this folder to production. Skip `llms.txt` with `tidypress build --no-llms-txt` or `capabilities.disable: ['llmsTxt']`.
